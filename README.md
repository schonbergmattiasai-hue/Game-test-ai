...
By default the script uses `assets/target.png`, which should be the blue bullseye/target icon from the provided screenshot. If it is missing, the script will attempt to download the image automatically from the shared URL. If the download fails, save the screenshot locally and pass the path:
